...
export const NavBar = ({ cursorEnabled, setCursorEnabled }) => {
  ...
  return (
    <header className='bg-aesthetic w-full h-[50px] px-6 fixed z-50'>
        <nav className='w-full h-full flex items-center justify-between'>
            <ul className='flex gap-10 justify-center items-center'>
                <li className='h-full flex justify-center items-center'>
                    <a href="#" className='uppercase font-primary text-bold-black hover-target'>
                        <img src="/logo.png" alt="logo" className='w-[50px] h-[50px] object-contain'/>
                    </a>
                </li>
                <li className='h-full'>
                    <a href="#" className='uppercase font-primary text-bold-black text-xs hover-target font-semibold hover:text-aesthetic-brown'>work</a>
                </li>
                <li className='h-full'>
                    <a href="#" className='uppercase font-primary text-bold-black text-xs hover-target font-semibold hover:text-aesthetic-brown'>about</a>
                </li>
                <li>
                    <a href="#" className='uppercase font-primary text-bold-black text-xs hover-target font-semibold hover:text-aesthetic-brown'>contact</a>
                </li>
            </ul>
            {
                deviceType !== "desktop" ? (
                ...
                <label className="relative inline-flex items-center cursor-pointer hover-target">
                    ...
                    />
                    <div className="w-10 h-5 bg-aesthetic border-[1px] rounded-full peer-checked:bg-aesthetic transition-all relative flex justify-center items-center">
                        <div
                            ref={iconRef}
                        ...
                        {cursorEnabled ? (
                            <div ref={circleRef} className="absolute top-0 left-0 right-0 bottom-0 flex justify-center items-center z-50">
                                <div className="relative w-[10px] h-[10px] border border-aesthetic rounded-full">
                                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[3px] h-[3px] bg-aesthetic rounded-full"></div>
                                </div>
                            </div>
                        ) : (
                            <div ref={mousePointerRef} className="absolute top-0 left-0 right-0 bottom-0 flex justify-center items-center z-50">
                                <FaMousePointer className="text-aesthetic text-[14px]" size={10}/>
                            </div>
                        )}
                        </div>
                    </div>  
